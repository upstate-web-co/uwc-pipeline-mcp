# @upstatewebco/uwc-pipeline-mcp

> MCP server that exposes a directory of Claude Agent Skills as callable tools — `list_skills` and `get_skill`. Pairs with [`@upstatewebco/uwc-skills`](https://github.com/upstate-web-co/uwc-skills-npm), but works against any Agent Skills-compliant directory.
...
The server is deliberately content-agnostic. The bundled [`@upstatewebco/uwc-skills`](https://github.com/upstate-web-co/uwc-skills-npm) is one valid skill source; anything that follows Anthropic's Agent Skills spec will work.

## Install

```bash
npm install -g @upstatewebco/uwc-pipeline-mcp
```

Or install locally and invoke via `npx`:

```bash
npx @upstatewebco/uwc-pipeline-mcp
```
...
2. `./node_modules/@upstatewebco/uwc-skills/skills`, if the package is installed in the working directory
...
```bash
claude mcp add uwc-pipeline-mcp -- npx @upstatewebco/uwc-pipeline-mcp
```
...
{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"list_skills","arguments":{}}}' | UWC_SKILLS_DIR=/path/to/skills npx @upstatewebco/uwc-pipeline-mcp
```
...
If you need pipeline tooling beyond skill browsing, the source of truth for those patterns is published as skill *content* in [`@upstatewebco/uwc-skills`](https://github.com/upstate-web-co/uwc-skills-npm) — load the relevant skill with `get_skill` and apply it yourself.
...
- **Skill bundle:** [`@upstatewebco/uwc-skills`](https://github.com/upstate-web-co/uwc-skills-npm)
